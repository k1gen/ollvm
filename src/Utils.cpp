...
CallBase *fixEH(CallBase *CB) {
  ...
  auto It = EHBlock->begin();
  while (isa<PHINode>(*It))
    ++It;
  Instruction *EHPad = &*It;

  const OperandBundleDef OB("funclet", EHPad);
  auto InsertPt = CB->getIterator();
  auto *NewCall =
      CallBase::addOperandBundle(CB, LLVMContext::OB_funclet, OB, InsertPt);
    ...
    if (PHINode *PHI = dyn_cast<PHINode>(I)) {
      for (unsigned int i = 0; i < PHI->getNumIncomingValues(); ++i) {
        ...
        if (ConstantExpr *CE =
          ...
          auto InsertIt = TI->getIterator();
          NewInst->insertBefore(InsertIt);
